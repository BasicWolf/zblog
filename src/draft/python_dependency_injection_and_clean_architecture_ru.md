...
Тогда попытаемся решить эту проблему посредством ООП:
...
Значит, необходим некий глобальный **контекст** который будет управлять
инициализацией и связыванием компонентов.
Но прежде чем перейти к построению такого контекста, давайте
немного порассуждаем о типах компонентов и о том, какую роль
они играют при связывании.
...
Её нельзя скрывать глубоко в недрах приложения.
Нам важно суметь **показать** предназначение программы
программисту читающему её код.
...
Наконец, мы показываем структуру нашего приложение. На Питоне
...
в некоем контексте `app_context` и с которым может взаимодействовать
