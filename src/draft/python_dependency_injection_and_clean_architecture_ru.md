...
Я очень надеюсь, что внимательный читателю уже ответил на этот вопрос
и ему даже не нужно читать дальше.
Если же нет - может в этом и моя вина, что к этому моменту связь между
DDD и Гексагональной архитектурой не столь очевидна.

Гексагональная архитектура позволяет
полностью изолировать бизнес-логику предметной области.
Название объектов и их взаимодействие перетекает из
универсального языка в код и обратно.




You don't need all of that, all you need is a running software (Evans, 11)
