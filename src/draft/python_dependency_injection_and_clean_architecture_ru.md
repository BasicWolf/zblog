...
через **ведущие** порты, на просторечье - API. Команды которые
отдаёт приложение поступают в **ведомые** порты - SPI (Service
Provider Interface).

А между портами и доменом сидят серсивы приложения (Application Services).
Они являются *связующим звеном* между различными сценариями
использования (use cases) и доменом с одной стороны и
...
Давайте же немного углубимся в детали и рассмотрим работу слоёв
по-отдельности и вместе.

Например, имеется ведущий порт AddItemToBasket
